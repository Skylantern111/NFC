...
export default function BackButton({ fallback = '/', label = 'Back', className = '' }) {
  ...
  return (
    <button
      type="button"
      onClick={onBack}
      className={`inline-flex items-center gap-1.5 text-sm font-medium text-slate-600 dark:text-slate-300 transition-colors hover:text-slate-900 dark:hover:text-slate-50 ${className}`}
    >
